...
router.get('/gracias', async (req, res) => {
  const PAGINA_GRACIAS = `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Pago confirmado — Valentia</title><meta name="robots" content="noindex"><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,560;0,9..144,700&family=Instrument+Sans:ital,wght@0,400;0,500;0,600;0,700&display=swap" rel="stylesheet"><style>:root{--ink:#20242E; --cream:#FBF2EA; --accent:#F14141; --accent-mid:#D93B3B;--accent-dim:rgba(241,65,65,.12); --text-body:rgba(32,36,46,.78);--text-muted:rgba(32,36,46,.58); --border:rgba(32,36,46,.10);--border-acc:rgba(241,65,65,.25); --serif:'Fraunces',Georgia,serif;--sans:'Instrument Sans','Helvetica Neue',Arial,sans-serif; --radius:6px;--mint:#2FAE7A; --mint-soft:#E3F5EC;}*{box-sizing:border-box;margin:0;padding:0}body{font-family:var(--sans);color:var(--ink);line-height:1.6;background:var(--cream);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:32px 20px}a{color:inherit}.card{max-width:560px;width:100%;background:#fff;border:1px solid var(--border);border-radius:14px;padding:44px 40px;box-shadow:0 30px 60px -40px rgba(32,36,46,.25)}.word{font-family:var(--serif);font-weight:700;font-size:15px;color:var(--accent);text-transform:uppercase;letter-spacing:.14em;margin-bottom:28px;display:block}.check{width:52px;height:52px;border-radius:100px;background:var(--mint-soft);display:flex;align-items:center;justify-content:center;margin-bottom:20px}.check svg{width:26px;height:26px}h1{font-family:var(--serif);font-weight:600;font-size:1.7rem;letter-spacing:-.015em;margin-bottom:12px}p{color:var(--text-body);font-size:15px}.lead{margin-bottom:28px}.help{background:var(--cream);border:1px solid var(--border);border-radius:var(--radius);padding:22px 24px;margin-top:8px}.help h2{font-family:var(--sans);font-weight:700;font-size:14px;text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px;color:var(--ink)}.help p{font-size:14px;margin-bottom:18px}.help p strong{color:var(--ink)}.btns{display:flex;gap:12px;flex-wrap:wrap}.btn{display:inline-flex;align-items:center;gap:8px;font-family:var(--sans);font-weight:700;font-size:.78rem;letter-spacing:.09em;text-transform:uppercase;padding:.85rem 1.3rem;border-radius:4px;text-decoration:none;transition:.2s ease}.btn-wa{background:#25D366;color:#fff}.btn-wa:hover{background:#1DA851}.btn-mail{background:transparent;color:var(--ink);border:1px solid var(--border)}.btn-mail:hover{border-color:var(--border-acc);color:var(--accent)}.foot{margin-top:30px;font-size:12.5px;color:var(--text-muted)}</style></head><body><div class="card"><span class="word">Valentia</span><div class="check"><svg viewBox="0 0 24 24" fill="none"><path d="M4 12.5L9.5 18L20 6" stroke="#2FAE7A" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></svg></div><h1>¡Pago confirmado!</h1><p class="lead">Tu web se está publicando automáticamente. Te avisaremos por email en cuanto esté lista (normalmente en pocos minutos).</p><div class="help"><h2>¿Algo no cuadra una vez esté en directo?</h2><p>Si al ver tu web ya publicada notas algún error o necesitas pedirnos alguna corrección, escríbenos por <strong>email</strong> o <strong>WhatsApp</strong> contándonos qué necesitas. No te preocupes — te atendemos en menos de 24 horas.</p><div class="btns"><a class="btn btn-wa" href="https://wa.me/34644036814" target="_blank" rel="noopener">WhatsApp</a><a class="btn btn-mail" href="mailto:info@valentiams.com?subject=Correcci%C3%B3n%20en%20mi%20web">Escribir email</a></div></div><p class="foot">© ${new Date().getFullYear()} Valentia · web.valentiams.com</p></div></body></html>`;router.get('/gracias', async (req, res) => {sendHtml(res, 200, PAGINA_GRACIAS);});
});
