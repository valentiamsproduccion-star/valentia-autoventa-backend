...
respuestas de Stripe y Anthropic (no requiere claves reales para comprobar
que el flujo entero funciona). Con claves reales, funciona igual pero con
IA y pagos de verdad.

## Por qué está escrito así

Dos decisiones de este código no son las que se tomarían con acceso normal
a internet y a npm, y conviene saberlo antes de tocarlo:

- **Sin Express ni SDKs de Stripe/Anthropic.** El entorno en el que se
  escribió esto no tenía acceso al registro de npm. Todo está construido
...
- **Base de datos en un archivo JSON** (`src/db/db.js`), no SQLite/Postgres.
  Funciona bien para probar el flujo y para un volumen bajo de clientes,
  pero **hay que sustituirla antes de manejar tráfico real** — ver
  "Antes de producción" más abajo. El resto del código solo conoce las
  funciones de `db.js` (`createClient`, `createOrder`, etc.), así que
  cambiar el motor de almacenamiento no debería tocar nada más.
...
Esto simula Stripe y Anthropic y ejercita el flujo completo (alta → vista
previa → checkout → webhook → publicación → enlace mágico → compra del
suplemento). Si termina con "✔ TODO EL FLUJO END-TO-END FUNCIONA
CORRECTAMENTE", el código está sano.

## Puesta en marcha real (con Stripe y Anthropic de verdad)
...
3. **Hosting**: elige dónde vive este backend (ver siguiente sección —
   importante, Hostinger no vale para esto).
4. Arranca con `node src/server.js` (o `npm start` si añades ese script).
...
- **Publicación del dominio/subdominio real, SSL, GA4, Search Console,
  ...
  servicio genera el HTML final y lo dejar guardado en disco (`SITES_DIR`)
  — no lo sube a ningún hosting ni activa un dominio, porque eso depende
  del proveedor que se decida en la reunión técnica.
...
- **Sustituir la base de datos JSON por Postgres o SQLite real.** El
  archivo único (`data/db.json`) no soporta escrituras concurrentes de
  verdad ni crece bien — es correcto para probar el flujo, no para
  servir tráfico de clientes reales.
...
- **Backups de `data/` y `sites/`** una vez haya clientes reales.
...
autoventa/
  src/
    ...
    db/
      db.js                  "base de datos" en un archivo JSON
    services/
      ...
      publish.js               genera y guarda el HTML final publicado
    prompts/
